...
```
C:\> windows-privesc-check2.exe

windows-privesc-check v2.0svn198 (http://pentestmonkey.net/windows-privesc-check)

Usage: C:\windows-privesc-check2.exe (--dump [ dump opts] | --dumptab | --audit) [examine opts] [host opts] -o report-file-stem
...
Options:
  ...
  report opts:
    ...
    -B EXPLOITABLE_BY_FILE, --exploitablebyfile=EXPLOITABLE_BY_FILE
                        Report privesc issues only for these user/groups
```