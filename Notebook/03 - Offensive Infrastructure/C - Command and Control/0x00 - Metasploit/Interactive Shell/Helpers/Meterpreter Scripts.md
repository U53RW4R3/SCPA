https://trustedsec.com/blog/benchmarking-self-hosted-llms-for-offensive-security# Metasploit Modules
...
```
>> session.shell_read("/path/to/file")
```

Execute shell commands (only meta shell and not `meterpreter`).
...
```
>> session.sys.process.execute("cmd.exe /c <commands>", nil, {'Hidden' => true, 'Channelized' => true})
```
...
- [rapid7: Metasploit Framework Python Dummy Auxiliary Modules](https://github.com/rapid7/metasploit-framework/blob/master/modules/auxiliary/example.py)

- [rapid7: Metasploit Framework Ruby Dummy Auxiliary Modules](https://github.com/rapid7/metasploit-framework/blob/master/modules/auxiliary/example.rb)
